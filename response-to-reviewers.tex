...
However, in our case there are several software packages that are directly
and indirectly related to the Demes format as summarized in Table 1.
We have instead added a link to the specification documentation in the abstract.
...
\ggcomment{Anything here we should clarify in the manuscript?}
...
requires errors be raised when models are ambiguous,
and explicitly documents the required behavior for edge cases.
...
We apreciate that the timescale we've chosen may at first be jarring
because it uses backwards-time values with forwards-time language.
But defining the timescale was fundamental choice that we didn't take lightly.
All decisions about Demes are publicly recored in github issues,
pull requests, and code experiments. Two discussions relevant to the
timescale are
...
On a more practical note, changing how the timescale is defined
at this stage of the Demes project would require substantial changes
to all of the Demes software and documentation.

\ggcomment{
Translation of models to forwards time simulators...
Refer to KT's comment above?
}

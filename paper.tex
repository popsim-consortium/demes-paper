...
\aprcomment{You could say that ``showcase'' also refers to the table that
includes demes support in inference software, not just the illustration
in the main text.}
\end{abstract}
...
we ensure consistency across simulation
...
C languages (Table~\ref{tab:software}, and discussed in
...
carrying capacities, implementations of \emph{hard} selection, or layering more
...
features of the underlying genome biology (Appendix~\ref{sec:appendix-scope}).
...
of parameter values.
While we have opted to restrict Demes to static model descriptions,
allowing parameterized distributions or parameter ranges 
is a direction of possible future work.

...
A standard format also serves to improve scientific reproducibility. Authors simply include their demographic model as a supplementary file, or upload it to a data repository, so that readers need not transcribe the model themselves. This removes the opportunity for error due to model misinterpretation or incorrect translation to a simulatable format. In the event that the Demes standard becomes outdated, published Demes files will be convertible to future formats with relative ease because the data model is unambiguously specified.

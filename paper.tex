...
In Figure \ref{fig:IM} we provide an example model in Demes YAML format.
...
There are three demes in the model, an ancestral deme named ``A'' which ceases
to exist at 100 generations ago, and demes ``X'' and ``Y'' that derive their
ancestry from ``A'' at this time.
...
and readers are instead directed to the tutorial included
in the online Demes specification
(\url{https://popsim-consortium.github.io/demes-spec-docs/}).

...
Consortium. \aprcomment{do we need ``developed by the PopSim Consortium''?}
The Demes data model precisely defines key parameters
...
across fields, default may be used to implicity assign a default value to
fields of the given type.  A default is superceded by an explicitly provided
value if given. Other values are inherited naturally following the progression
of time. For example, if an epoch \texttt{start\_size} is not provided, it is
assumed to be equal to the \texttt{end\_size} of the previous epoch. This also
means that the first epoch of each population must specify the initial size.
...
practise parsers output an object model that is suitable for the particular
...
Demes is meant to interface with a large number of simulation and inference
...
population genetic software, many features of underlying genome biology are
...
deterministic number of individuals.  Finally, as Demes assumes the
exchangeability of individuals within populations, Demes does not provide a way
to define geographic spatial structure within populations.

Demes version 1.0 is intended to provide a stable and minimal specification
that is compatible with a wide range of simulation softwares. Beginning with
version 1.0, a Demes-specified model is expected to provide the version of
Demes for which it is valid, and if unprovided, it should be a valid model
under the default version, 1.0. Future extensions of the Demes specification
will be released with versions following semantic versioning guidelines [cite
or url?], and downstream software that incorporate Demes are expected to
indicate and check Demes version compatibility.

...
\aprcomment{what exactly do we mean by ``definitive''?}
...
Below we detaail the population genetics model in 
...
The extended figures in Appendix~\ref{sec:appendix-figures} provides
...
or through processes of continuous or pulse migrations.
...
\aprcomment{previous sentence changed to allow for selfing/cloning}
...
reproduce and die simultaneously) and the expected frequency (at birth)
of the $i^{th}$ allele currently at frequency $p_i$
one generation in the next generation is $p_iw_i/\bar{w}$,
...
described in \cite[][chapter 3]{wakeley2008-hd}.
...
% In the \texttt{Discussion}, we discuss alternative models compatible
% with this framework and outline the expectations for tools implementing
% them.
\aprcomment{As such, this model excludes scenarios such as ``hard selection,''
in which population sizes are dependent on a population's mean fitness, or
stochastic fluctuations in population size, such as defining ``carrying capacities''.}
Many forwards and backwards time simulators currently implement this model
\citep[e.g.,][\aprcomment{and more\dots?}]{hudson2002generating,gutenkunst2009inferring,excoffier2011fastsimcoal,jouganous2017inferring,haller2019slim,thornton2019-nu}.
...
% \krtcomment{
%     We need to revisit the paragraph(s) below.
%     We do not assume individuals are exchangeable, because we do not
% 	require that all individuals have the same fitness.
%     See, e.g., p 60 of Wakeley for why fitness == exchangeability
% 	and why that is required for the standard coalescent.
% }
% \mhcomment{The `exchangeability' assumption also breaks down under 
% 	different reproductive modes, since the sampling of the alleles
%     affects coalescent times. 
% 	For example, in a highly selfing species the coalescent time for two different alleles 
% 	within an individual is much lower than two alleles from different individuals.
%     I've triedto rewrite the below paragraph to update and clarify
%     the reproduction parameters in line with the discussions on Github.
%     Finally, 'rates' imply events occur as Poisson-like 
% 	processes over continuous time, whereas the values used here instead define the 
% 	\emph{fraction} of reproductions of each type.
%     Shall we rename to selfing/clonal `fractions',
...
and $S=0$, selfing may still occur with probability $1/N$).
...
\aprcomment{
    Do we still want to say something along these lines?
    ``However, consuming software is free to ignore these parameters
    and model the genetics within
    the framework of a soft selection model.
    We don't view these as limitations. Rather, this is what most people
